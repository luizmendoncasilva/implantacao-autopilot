...
(function (global) {
  ...
  /* ---- "Salvar alterações?" — dialog de confirmação próprio, sem menção ao
     Cockpit, para ações Autopilot nativas que não têm integração real com o
     Cockpit (ex.: Contador Responsável, Bloco 1.6-b — ver
     docs/RN-RF_CadastroEmpresasAuxiliares.md, seção 4-A). Ao contrário de
     confirmarReflexoCockpit acima (usado pelo fluxo de edição com reflexo no
     Cockpit, hoje sem ponto de entrada em v1), esta ação é real e não deve
     sugerir uma sincronização com o Cockpit que não existe. ---- */
  function confirmarSalvarNativo(onConfirmar, onDescartar) {
    showConfirmModal({
      title: "Salvar alterações?",
      message: "As alterações realizadas nesta empresa não têm reflexo no Cockpit — são exclusivas do Autopilot.",
  ...
  global.ApEmpresaShell = { resolveEmpresaOrRedirect, mount, tabLabel, situacaoBadge, toast, abrirDrawer, fecharDrawer, confirmarReflexoCockpit, confirmarSalvarNativo, boot };
})(window);
